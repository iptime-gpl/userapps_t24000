 ...
 *
 * $Id: pptpd.c,v 1.4 2012/12/10 01:02:15 rtl3 Exp $
 */
...
int main(int argc, char **argv)
{
	...
	while (1) {
...
#ifdef BCRELAY
		c = getopt_long(argc, argv, "b:c:dfhl:o:p:s:t:v", long_options, &option_index);
#else
		c = getopt_long(argc, argv, "c:dfhl:o:p:s:t:v", long_options, &option_index);
#endif
		if (c == -1)
			break;
		...
		if (c == 0)
#ifdef BCRELAY
			c = "bcdfhlopstv"[option_index];
#else
			c = "cdfhlopstv"[option_index];
#endif
		switch (c) {
#ifdef BCRELAY
		case 'b':
