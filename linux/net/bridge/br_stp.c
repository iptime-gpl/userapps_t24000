...
extern int mv_salsa2_set_stp_port_state(int stpidx, int port, int state);
#endif
...
#ifdef CONFIG_PORT_TRUNK_FEATURE
static int trunk_first_port_no(struct net_bridge *br, int trunk_map)
{
	struct net_bridge_port *p;

	list_for_each_entry(p, &br->port_list, list)
	{
		if (trunk_map & (1<<p->port_no))
			return p->port_no;
	}
	return -1;
}

void br_trunk_member_port_state_selection(struct net_bridge *br, int trunk_map, int state)
{
	struct net_bridge_port *p;

	list_for_each_entry(p, &br->port_list, list)
	{
		if (trunk_map & (1<<(p->port_no-1)))
		{
			p->state = state;
#ifdef CONFIG_EFM_ROUTER_PATCH
			mv_salsa2_set_stp_port_state(0, p->port_no-1, p->state);
#endif
			br_log_state(p);
		}
	}
...
#ifdef CONFIG_PORT_TRUNK_FEATURE
		if (p->trunk_map &&
		   (trunk_first_port_no(p->br, p->trunk_map) != p->port_no))
				continue;
#endif
...
#ifdef CONFIG_PORT_TRUNK_FEATURE
		if (p->trunk_map)
			br_trunk_member_port_state_selection(p->br, p->trunk_map, p->state);
#endif
	}
}
