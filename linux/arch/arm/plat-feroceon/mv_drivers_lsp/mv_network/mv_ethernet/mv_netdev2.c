...
static INLINE int eth_rx(struct net_device *dev, unsigned int work_to_do, int queue)
{
	...
	{ // EFM DEBUG
	int ii;
	int len = pPktInfo->pFrags->dataSize - ETH_MV_HEADER_SIZE;
	printk("== Rx packet %d bytes (%s) ORG ==, display max 60 bytes of packet\n", len, dev->name);
	printk("%02x %02x\n", data[0], data[1]);
	if (len > 60) len = 62;
	for (ii = 2; ii < len; ii++)
	{
	...
	if (!strcmp(dev->name, "eth0"))
	{
		unsigned char new_header[4];
    		unsigned char *dsa_header = &data[2+12]; // 2bytes-padding, 6bytes-dstmac, 6bytes-srcmac

		#define DSA_HLEN 	4

		if (data[2] == 0x01 && data[3] == 0x80 && 
		    data[4] == 0xc2 && data[5] == 0x00 && 
		    data[6] == 0x00 && data[7] == 0x00) //BPDU
		{
			size_t size = (data[18] << 8) | data[19];	
			unsigned char *ptr;

			memcpy(data+(ETH_ALEN*2)+2,
				data+(ETH_ALEN*2)+DSA_HLEN + 2,
				size+2);
			ptr = data + 2 + ETH_HLEN + size;
		...
		else
		{
			...
			new_header[3] = ((dsa_header[1]&0xf8)>>3)+1;
	...
	int ii;
	int len = pPktInfo->pFrags->dataSize - ETH_MV_HEADER_SIZE;
	printk("== Rx packet %d bytes (%s) ==, display max 60 bytes of packet\n", len, dev->name);
	printk("%02x %02x\n", data[0], data[1]);
	if (len > 60) len = 62;
	for (ii = 2; ii < len; ii++)
	{
...
static int eth_tx( struct sk_buff *skb , struct net_device *dev )
{
    ...
    */
	...
	if (!strcmp(dev->name, "eth0")) // to LAN
	{
		...
		if ((dsa_header[0] == 0x08 && dsa_header[0] == 0x00) ||
		    (dsa_header[0] == 0x08 && dsa_header[0] == 0x06))
		{
			; // no header
		}
#if 0
	...
	int ii;
	int len = skb->len;
	printk("== Tx packet %d bytes (%s) ==, display max 60 bytes of packet\n", skb->len, dev->name);
	if (len > 60) len = 60;
	for (ii = 0; ii < len; ii++)
	{
		if (ii%16 == 0) printk("\n");
...
static int __init mv_eth_init_module( void ) 
{
	...
	mvEthPhyRegRead(1, 0x0, &val16);
	printk("==> wan phy reg 0 value : %0x \n", val16);
	mvEthPhyRegRead(1, 0x1, &val16);
	printk("==> wan phy reg 1 value : %0x \n", val16);
	mvEthPhyRegRead(1, 0x2, &val16);
	printk("==> wan phy reg 2 value : %0x \n", val16);
	mvEthPhyRegRead(1, 0x3, &val16);
	printk("==> wan phy reg 3 value : %0x \n", val16);
