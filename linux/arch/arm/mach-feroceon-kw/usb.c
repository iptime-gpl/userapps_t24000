...
int check_usb_devicemode_by_power(int ms_times)
{
        int i;

	printk("Check USB Mode Detecting ....");
        SetUSBControl(0);
	mdelay(1000);
        for(i=0;i<ms_times;i++)
        {
                if(!GetUSBPower())
                {
                        SetUSBControl(1);
...
#ifdef CONFIG_EFM_GPIO
	isHost = check_usb_devicemode_by_power(100)?0:1;
	SetUSBControl(isHost);
#else
